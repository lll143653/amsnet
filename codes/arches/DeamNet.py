...
class ConvLayer1(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, padding=(
            kernel_size-1)//2, stride=stride)
...
class Encoding_block(nn.Module):
    def __init__(self, base_filter, n_convblock):
        ...
        for i in range(self.n_convblock-1):
            modules_body.append(
                ConvLayer(base_filter, base_filter, 3, stride=1))
    ...
    def forward(self, x):
        ...
        if self.upsample:
            x_in = torch.nn.functional.interpolate(
                x_in, scale_factor=self.upsample)
...
class upsample1(nn.Module):
    def __init__(self, base_filter):
        ...
        self.ConvTranspose = UpsampleConvLayer(
            base_filter, base_filter, kernel_size=3, stride=1, upsample=2)
        self.cat = ConvLayer1(base_filter*2, base_filter,
...
class Attention_unet(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.conv_du = nn.Sequential(
            ConvLayer1(in_channels=channel, out_channels=channel //
                       reduction, kernel_size=3, stride=1),
            nn.ReLU(inplace=True),
            ConvLayer1(in_channels=channel // reduction,
                       out_channels=channel, kernel_size=3, stride=1),
            nn.Sigmoid()
        )
        self.cat = ConvLayer1(in_channels=channel * 2,
                              out_channels=channel, kernel_size=1, stride=1)
        self.C = ConvLayer1(in_channels=channel,
                            out_channels=channel, kernel_size=3, stride=1)
        self.ConvTranspose = UpsampleConvLayer(
            channel, channel, kernel_size=3, stride=1, upsample=2)  # up-sampling
...
class transform_function(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.ext = ConvLayer1(in_channels=in_channel,
                              out_channels=out_channel, kernel_size=3, stride=1)
...
class Inverse_transform_function(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.ext = ConvLayer1(in_channels=in_channel,
                              out_channels=out_channel, kernel_size=3, stride=1)
        self.pre = torch.nn.Sequential(
            ConvLayer1(in_channels=in_channel,
                       out_channels=in_channel, kernel_size=3, stride=1),
            nn.ReLU(inplace=True),
            ConvLayer1(in_channels=in_channel,
                       out_channels=in_channel, kernel_size=3, stride=1),
        )
