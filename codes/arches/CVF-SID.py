...
class GenClean(nn.Module):
    def __init__(self, channels=3, num_of_layers=17):
        ...
        layers.append(nn.Conv2d(in_channels=features,
                      out_channels=channels, kernel_size=1, padding=0))
...
class GenNoise(nn.Module):
    def __init__(self, NLayer=10, FSize=64):
        ...
        for i in range(NLayer-1):
            m.append(nn.Conv2d(FSize, FSize,
                     kernel_size=kernel_size, padding=padding))
        ...
        for i in range(4):
            gen_noise_w.append(
                nn.Conv2d(FSize, FSize, kernel_size=kernel_size, padding=padding))
    ...
    def forward(self, x, weights=None, test=False):
        ...
        m_w = torch.mean(torch.mean(noise_w, -1), -
                         1).unsqueeze(-1).unsqueeze(-1)
        noise_w = noise_w-m_w
        m_b = torch.mean(torch.mean(noise_b, -1), -
                         1).unsqueeze(-1).unsqueeze(-1)
        noise_b = noise_b-m_b
